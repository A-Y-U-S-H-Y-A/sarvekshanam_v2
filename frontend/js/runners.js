      ...
      <div class="group-card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
          <h3 class="group-card-header" style="border:none;padding:0;margin:0;">
            🏢 ${Utils.escHtml(g.name)}
            <span class="status-badge status-online">Active</span>
          </h3>
          <span style="font-family:var(--font-mono);font-size:0.62rem;color:var(--fg-4);">${Utils.escHtml(g.manifest_hash ? g.manifest_hash.substring(0,8) : '')}</span>
        </div>
        <div>
    ...
    listEl.innerHTML = this.list.map(r => `
      <div class="runner-card">
        <div class="runner-card-header">
          <span class="runner-name">${Utils.escHtml(r.name)}</span>
          <div style="display:flex;gap:6px;align-items:center;">
            ${r.group ? `<span class="status-badge">${Utils.escHtml(r.group)}</span>` : ''}
            <span class="status-badge status-${Utils.escHtml(r.status || 'offline')}">${Utils.escHtml(r.status || 'unknown')}</span>
          </div>
        </div>
        <div >
          <p class="runner-url"><strong style="color:var(--fg-4);font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;">URL:</strong> ${Utils.escHtml(r.url)}</p>
          <p class="runner-stat"><strong style="color:var(--fg-4);font-size:0.6rem;text-transform:uppercase;letter-spacing:0.1em;">Last Seen:</strong> ${Utils.escHtml(r.last_seen_at ? new Date(r.last_seen_at).toLocaleString() : 'Never')}</p>
          <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border);">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:4px;">
              ${(r.modules && r.modules.length > 0) ? r.modules.map(m => `<span style="font-family:var(--font-mono);font-size:0.6rem;padding:2px 6px;background:var(--surface);border:1px solid var(--border);color:var(--fg-3);">${Utils.escHtml(m.name)}</span>`).join('') : '<span style="font-family:var(--font-mono);font-size:0.68rem;color:var(--fg-4);font-style:italic;">None/Disconnected</span>'}
            </div>
          </div>
        </div>
        <div style="padding:10px 16px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;">
          <button class="btn btn-ghost btn-sm" onclick="Runners.delete('${Utils.escHtml(r.id)}')">🗑 Delete</button>
        </div>
      </div>
    `).join('');
  },
